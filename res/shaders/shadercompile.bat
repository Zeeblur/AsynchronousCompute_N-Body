dir
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V phong.vert -o vert_phong.spv
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V phong.frag -o frag_phong.spv
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V multiple.vert 
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V simple.frag
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V nbodyDouble.comp -o dcomp.spv
C:/VulkanSDK/1.0.65.1/Bin/glslangValidator.exe -V nbody.comp -o comp.spv

pause